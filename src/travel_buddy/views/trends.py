...
@trends_blueprint.route("/trends/add_car", methods=["GET"])
@limiter.limit("15/minute")
def show_car():
    if "username" not in session:
        return redirect("/")
    ...
    fuel_used_1_month = helper_routes.calculate_fuel_used(1000, float(details.get("mpg")))
    fuel_cost_1_month = helper_routes.calculate_fuel_cost(fuel_used_1_month, fuel_price)
    co2_emissions_1_month = helper_routes.generate_co2_emissions(1609340, "driving", details.get("fuel"))
